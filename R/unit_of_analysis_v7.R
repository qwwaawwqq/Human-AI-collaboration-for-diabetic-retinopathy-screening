## Table S14: unit-of-analysis sensitivity (largest-table rule) on current data. Run from repo root.
...
osa0$.n <- osa0$tp1+osa0$fn1+osa0$fp1+osa0$tn1
NAD<-1000; NBURN<-10000; NITER<-40000; NTHIN<-40
fit <- function(osa, grp, label){
  ...
  jm<-jags.model(textConnection(model_string),data=dl,n.chains=4,n.adapt=NAD,quiet=TRUE)
  update(jm,NBURN)
  M<-as.matrix(coda.samples(jm,c("sens","spec","youden"),n.iter=NITER,thin=NTHIN))
  Y<-cbind(M[,"youden[1]"],M[,"youden[2]"],M[,"youden[3]"]) # AI,Human,Collab
  ranks<-t(apply(Y,1,function(r) rank(-r,ties.method="min")))
  sucra<-(3-colMeans(ranks))/2; pbest<-tabulate(max.col(Y),3)/nrow(Y)
  dCA<-Y[,3]-Y[,1]; arms<-c("AI","Human","Collab")
  cat(sprintf("\n### %s  | collab tables=%d | studies=%d\n",label,sum(osa$test=="human_with_ai"),dl$ns))
  for(k in 1:3) cat(sprintf("  %-7s Se=%.3f Sp=%.3f Youden=%.3f [%.3f,%.3f] SUCRA=%.2f P(best)=%.2f\n",
     ...
     mean(Y[,k]),quantile(Y[,k],.025),quantile(Y[,k],.975),sucra[k],pbest[k]))
...
oN<-function(grp){o<-osa0[order(paste(osa0[[grp]],osa0$test),-osa0$.n),];o[!duplicated(paste(o[[grp]],o$test)),]}
fit(osa0,"dataset_id","Base (dataset_id, all)")
fit(osa0,"authoryr","By publication (all)")
fit(oN("dataset_id"),"dataset_id","One per dataset x arm (largest)")
fit(oN("authoryr"),"authoryr","One per publication x arm (largest)")
cat("\nDONE\n")
